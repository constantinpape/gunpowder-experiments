...
def cremi_unet(name='unet', sample_to_isotropy=False):
    ...
    outputs = networks_old.unet(raw_batched, initial_fmaps, fmap_increase, downsample_factors)

    # apply conv pass to all scale outputs
    affs_batched = [networks_old.conv_pass(out,
                                           kernel_size=1,
                                           num_fmaps=n_channels,
                                           num_repetitions=1,
                                           activation='sigmoid') for out in outputs]
    ...
    # TODO implemet multiscale loss
    loss = networks_old.multiscale_loss(gt_affs, affs, loss_weights)
