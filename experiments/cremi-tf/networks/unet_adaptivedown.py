...
def unet_adapt(
    ...
    Args:
        ...
        fmap_inc_factor:
            ...
            ``k*fmap_inc_factor**l``.
        downsample_factors:
            List of integers used to down- and up-sample the
            feature maps between layers in xy. Down-and up-sampling along z starts
            once the data is close to being isotropic
        anisotropy_factor:
            resolution of z / resolution of xy
        activation:
            Which activation to use after a convolution. Accepts the name of any
            tensorflow activation function (e.g., ``relu`` for ``tf.nn.relu``).
        layer:
    ...
    # downsample
    # compute full_downsample_factor
    downsample_factor_3d=[1,downsample_factors[layer], downsample_factors[layer]]
    if abs(reduce(mul, downsample_factors[:layer+1],1)-anisotropy_factor)>abs(reduce(mul, downsample_factors[:layer],
                                                                                     1)-anisotropy_factor):
        downsample_factor_3d[0]=downsample_factors[layer]
